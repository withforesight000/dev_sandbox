...
resolver configuration: `scutil --dns` on macOS, and `/etc/resolv.conf` with an
optional `resolvectl dns` fallback on Linux. Loopback and stub resolver
addresses are not forwarded because they are not reachable from the nested
container namespace.

The `docker` service does not repeat host DNS discovery. It requires the
validated value generated by `prepare-mounts`. If automatic detection is not
sufficient, set `ROOTLESS_DOCKER_DNS` to a comma-separated list of resolver
addresses reachable from Docker's Linux VM before reopening the Dev Container:
